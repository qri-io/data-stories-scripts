...
const collectData = async (timestamps) => {
  // create writebuffer
  const output = fs.createWriteStream('./output/stops.csv')

  // loop over timestamps
  for ( const [i, timestamp] of timestamps.entries()) {
    // read the file, parse json array of objects
    const fileBuffer = await readFile(`./data/stops-${timestamp}.json`)
    let data = JSON.parse(fileBuffer)

    // append the timestamp to each observation
    // some don't have all fields, make sure it's an empty string
    data = data.map(d => ({
      timestamp,
      error: '',
      message: '',
    ...
    if (timestamp === '1580210101') console.log(data)
